...
let allCaps = runners.map(function (array) {
...
// Problem 1 - oh, no our fun-run got rained out! extract and email and first name for each donor so your big boss can send out a rescheduled date. 
let emailNames = [];
runners.forEach(function (array) {
    emailNames.push("first name: " + array.first_name + " email: " + array.email)
});
console.log(emailNames);

// Problem 2 - big boss started a high rollers club. produce an array of people who have donated more than 200 bucks

let highRollers = [];
runners.filter(function (runner) {
    if (runner.donation >= 200) {
        highRollers.push(runner.first_name + " " + runner.last_name);
    }
});
console.log(highRollers);
// Problem 3 - reduce reduce reduce!!! the news wants to know what the average donation amount was for reporting purposes. the big boss is sweating because he hates math. write him a function that pulls out the mean amount and maybe you'll get a promotion!

let ticketMean = () => {
    let ticketPriceTotal = runners.reduce(function (total, runner) {
        return total + runner.donation;
    }, 0);
    let ticketPriceAverage = ticketPriceTotal / runners.length;
    return ticketPriceAverage;
};
console.log(ticketMean());